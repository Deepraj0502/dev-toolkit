...
import { Terminal, Send, Server, Network, Activity, AlertTriangle, Loader2 } from "lucide-react";
...
export default function NetcatTesterPanel() {
  const [jumpHost, setJumpHost] = useState(ALLOWED_SERVERS[3]); // default .25
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-1.5">
            <label className="text-xs uppercase tracking-widest text-slate-400 font-bold flex items-center gap-1.5">
              <Server className="h-3.5 w-3.5 text-indigo-400" /> Origin Jump Server
            </label>
            <select
              value={jumpHost}
          ...
          <div className="space-y-1.5">
            <label className="text-xs uppercase tracking-widest text-slate-400 font-bold flex items-center gap-1.5">
              <Activity className="h-3.5 w-3.5 text-indigo-400" /> Destination IP / Host
            </label>
            <input
              type="text"
              value={destIp}
              onChange={(e) => setDestIp(e.target.value)}
              placeholder="e.g. 10.150.22.4"
              className="w-full rounded-xl border border-slate-800 bg-slate-900 px-3.5 py-2.5 text-sm text-slate-100 font-mono placeholder:text-slate-600 focus:outline-none focus:border-indigo-500"
            />
          </div>

          <div className="space-y-1.5">
            <label className="text-xs uppercase tracking-widest text-slate-400 font-bold flex items-center gap-1.5">
              <Terminal className="h-3.5 w-3.5 text-indigo-400" /> Dest Port
            </label>
            <input
              type="number"
              value={destPort}
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            placeholder="Enter the payload string (ISO 8583, XML, JSON). It will be piped to the server followed by an 'Enter' (\n) carriage return."
            rows={4}
        ...
        >
          ...
          {isExecuting ? "Pinging Target Server..." : "Execute Netcat Test"}
        </button>
